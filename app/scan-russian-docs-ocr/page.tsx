...
import { scanPassportRussianDocsOcr, type ScanResponse } from "@/lib/api/passport";
...
export default function ScanRussianDocsOcrPage() {
  ...
  const handleScan = async () => {
    if (!file) return;
    console.info("[scan-russian-docs-ocr-page] scan:start", { modelFormat, device, checkQuality });
    setScanning(true);
    setError(null);
    try {
      const payload = await scanPassportRussianDocsOcr(file, {
        modelFormat,
        device,
        checkQuality,
        imgSize: 1500,
      ...
      <div className="relative mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
        <section className="mb-8 rounded-3xl border border-slate-200/90 bg-white p-6 shadow-md shadow-slate-900/5 sm:p-9">
          ...
          <p className="mb-6 text-sm text-slate-600">
            Проверка Pipeline из <a className="text-blue-600 underline" href="https://github.com/protei300/RussianDocsOCR" target="_blank">protei300/RussianDocsOCR</a>: детекция документа, восстановление геометрии, поля и OCR.
          </p>
          ...
          <div
            ...
            onClick={() => document.getElementById("russian-docs-ocr-file")?.click()}
            className={[
